...
def load_strategy(path, capital=1000, risk_per_trade=0.02):
    try:
        ...
        sys.stderr.write(f"Error cargando estrategia: {str(e)}\n")
        sys.exit(1)
...
async def run_symbol(symbol, timeframe, strategy_path, capital, risk_per_trade, is_real, max_candles=100):
    # Binance usa minúsculas para streams
        ...
        try:
            async with websockets.connect(url) as ws:
                async for msg in ws:
                    ...
                    # Solo procesamos si la vela se cierra
                    if not k["x"]:
                        continue

                    # Actualización de datos (incluyendo columnas para indicadores técnicos)
            ...
            sys.stderr.write(f"Error en stream {symbol}: {str(e)}\n")
...
# =========================
def main():
    # Leer el JSON de configuración que envía Java
    ...
    try:
        ...
        sys.stderr.write(f"Fallo en el motor RT: {str(e)}\n")
        sys.exit(1)
